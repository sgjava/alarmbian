...
# installs the matching targeted cuda-toolkit package on Ubuntu 24.04 safely.
#
# Steven P. Goldsmith
# sgjava@gmail.com
...
log_info "Starting Driver-Aware CUDA Toolkit installation for Ubuntu 24.04..."

# 1. Verify nvidia-smi is present to determine version requirements
if ! command -v nvidia-smi &> /dev/null; then
    log_error "nvidia-smi could not be found. Please ensure your nvidia-driver-595-open is fully installed first."
    exit 1
fi

# Extract the maximum supported CUDA version (e.g., "13.2")
DETECTED_VERSION=$(nvidia-smi | grep -o "CUDA Version: [0-9 wave]*\.[0-9]*" | head -n 1 | awk '{print $3}')
...
# Convert the dot format to apt package hyphen format (e.g., "13.2" -> "13-2")
...
log_info "Targeted package chosen to prevent driver corruption: ${TARGET_PACKAGE}"

# 2. Ensure essential system building blocks and headers are present
...
# 3. Configure NVIDIA Repository Pinning Preference
log_info "Setting up repository preferences pinning for CUDA..."
...
wget -q https://developer.download.nvidia.com/compute/cuda/repos/ubuntu2404/x86_64/${KEYRING_FILE}
dpkg -i ${KEYRING_FILE}
rm -f ${KEYRING_FILE}

# Refresh lists now that the official CUDA repository source is available
apt-get update
...
if ! apt-get install -y "${TARGET_PACKAGE}"; then
    log_error "Failed to install ${TARGET_PACKAGE}. Falling back to verifying package availability..."
    exit 1
fi
...
# Inject to native dynamic linker search configurations
echo "${ACTIVE_CUDA}/lib64" > "${LD_SO_CONF_DIR}"
ldconfig

# --- Verification & Summary ---
log_info "========================================================"
log_info " CUDA Toolkit Setup Completed Successfully! "
log_info "========================================================"
log_info "System-wide variable setup complete targeted to: ${ACTIVE_CUDA}"

# Source execution path to evaluate verification within the running shell instance
export PATH="${ACTIVE_CUDA}/bin:$PATH"

if command -v nvcc &> /dev/null; then
    log_info "Compiler Validation: $(nvcc --version | grep 'release')"
else
    log_warn "nvcc was not found immediately in this current shell instance. Please open a new terminal."
fi
